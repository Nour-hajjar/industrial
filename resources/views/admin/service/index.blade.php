                ...
                <div class="card">
                    <div class="card-header">
                       {{trans('app.service')}}
                        ...
                        <a href="{{ route('services.create') }}" class="btn btn-sm btn-primary float-right">{{trans('app.Add_New')}}</a>
						@endcan
                    </div>
                            ...
                            @foreach($services as $service)
                                <tr>
                                    ...
                                    <td>
                                         <div class="row">
                                        <div class="col-sm-4"   >
                                            <a class="btn btn-sm  btn-success col-sm-12" href="{{ route('services.show',$service->id) }}">{{trans('app.show')}}</a>
                                        </div>
                                        <div class="col-sm-4"  >
                                   	 @can('directory-edit')
                                        ...
                                        <button class=" btn btn-sm btn-danger col-sm-12 openModal" id='{{ "openModal".$service->id }}' onclick='{{ "runme(openModal".$service->id.")" }}' >{{trans('app.delete')}}</button> @endcan
                                        <div class=" modal" id='{{ "m_openModal".$service->id }}'>
                                        <div class="modalContent">
                                        <span class="close" onclick='{{ "hideme(openModal".$service->id.")" }}' id='{{ "c_openModal".$service->id }}'>×</span>
                                        ...
                                        <button class="del" onclick='{{ "hideme(openModal".$service->id.")" }}'>{{trans('app.delete')}}</button>
                                        {!! Form::close() !!}
                                        <button class="cancel" onclick='{{ "hideme(openModal".$service->id.")" }}'>{{trans('app.cancel')}}</button>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
